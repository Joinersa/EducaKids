package br.com.projetoslabex.educakids.activities;

import android.app.AlertDialog;
import android.content.Intent;
import android.os.Bundle;
import android.support.v7.app.AppCompatActivity;
import android.support.v7.widget.Toolbar;
import android.view.View;
import android.view.Menu;
import android.view.MenuItem;
import android.widget.ImageView;

import br.com.projetoslabex.educakids.R;

public class MainActivity extends AppCompatActivity {

    ImageView btAlfabeto;
    ImageView btMatematica;
    ImageView btRanking;

    @Override
    protected void onCreate(final Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        inicializarViews();

        btAlfabeto.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(MainActivity.this, LetrasActivity.class);
                startActivity(intent);
            }
        });

        btMatematica.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(MainActivity.this, MatematicaActivity.class);
                startActivity(intent);
            }
        });

        btRanking.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(MainActivity.this, RankingActivity.class));
            }
        });
    }

    private void inicializarViews() {
        btAlfabeto = (ImageView) findViewById(R.id.btAlfabeto);
        btMatematica = (ImageView) findViewById(R.id.btMatematica);
        btRanking = (ImageView) findViewById(R.id.bt_ranking);
    }



    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        // Inflate the menu; this adds items to the action bar if it is present.
        getMenuInflater().inflate(R.menu.menu_main, menu);
        return true;
    }


    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        // Handle action bar item clicks here. The action bar will
        // automatically handle clicks on the Home/Up button, so long
        // as you specify a parent activity in AndroidManifest.xml.
        int id = item.getItemId();

        //noinspection SimplifiableIfStatement
        if (id == R.id.action_sobre) {

            AlertDialog.Builder sobre = new AlertDialog.Builder(MainActivity.this);
            sobre.setTitle("Sobre");

            View view = getLayoutInflater().inflate(R.layout.layout_sobre, null, false);
            sobre.setView(view);
            sobre.setPositiveButton("OK", null);
            sobre.show();

            return true;
        }

        return super.onOptionsItemSelected(item);
    }
}